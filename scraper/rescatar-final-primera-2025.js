// Rescate puntual: Final de Primera FEB 2025/26 (Final Four, 07/06/2026),
// que la FEB no enlaza en la vista de la fase pero existe por ID directo.
...
const ID = '2513900';
const FASE = 'Play-offs Final';
const JORNADA = 'Final(07/06/2026)';
const FECHA = '07/06/2026';
const DIR = path.join('data', 'raw', 'primerafeb', '2025', '_fases', 'Play-offs-Final');
...
async function main() {
  ...
  // Nombres de equipo en orden del DOM (local primero, visitante después)
  ...
  fs.writeFileSync(path.join(DIR, `${ID}.json`), JSON.stringify({
    id: ID, fase: FASE, jornada: JORNADA, temporada: '2025',
    celdas: [`${local} -\n\t${visitante}`, resultado, FECHA, ''],
    resultado, boxscore
  }, null, 1));

  console.log(`✔ Final guardada: ${local} ${resultado} ${visitante}`);
  console.log(`  Esperado: MOVISTAR ESTUDIANTES 97-100 LEYMA CORUÑA (gana Leyma en prórroga)`);
}
